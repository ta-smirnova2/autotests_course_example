...
# Здесь пишем код
class PersonInfo:
    def __init__(self, Fam_Name: str, Age: int, *args):
    ...
    def short_name(self):
        sp = self.fam_name.find(' ')
        l = self.fam_name[sp+1:]+' '+self.fam_name[:1]+'.'
        return l
    ...
    def path_deps(self):
        ...
        for i in self.work:
            str_work += i+' --> '
        if str_work != '':
            str_work = str_work[:len(str_work)-5]
        ...
        for s in self.work:
            ...
            for i in lst:
                if i in s_dict:
                    ...
                    s_dict.setdefault(i, 1)
        s_list = []
        for i in s_dict.values():
            s_list.append(i)
        s_list.sort()
        if len(s_list)>0 and len(s_list)<=3:
            s_sum = sum(s_list)
        elif len(s_list)>3:
            s_sum = sum(s_list[len(s_list)-3:])
        else:
            s_sum = 0
        s_sum = 1337*self.age*s_sum

        return s_sum

# my_person = PersonInfo('Александр Шленский', 32, 'УК')
# print(my_person.short_name())
# print(my_person.path_deps())
# print(my_person.pnew_salary())
